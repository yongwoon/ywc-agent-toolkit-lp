...
- **`README.md` (5개 로케일)**:
  ...
  - (선택, 실행자 재량) 새 인프라 파이프라인을 가리키는 신규 Quick Links 행 1개 추가 — 예: "Design or review cloud infrastructure before applying it" → `[14](./17-infrastructure-and-cloud.md)`
...
- README.md의 선택적 신규 Quick Links 행 추가 여부는 스펙 Open Questions에서 실행자 재량으로 남겨져 있다 — 추가하지 않아도 AC4는 충족된다.
