# 000006-020-config-guidebook-i18n-en

## Purpose
`src/content/guidebook/ko/`(이 task 실행 시점에 존재하는 모든 페이지)의 한국어 guidebook 콘텐츠를 영어(en)로 번역하고, `src/content/guidebook/en/`에 배치하며, guidebook route/sidebar가 en locale도 서빙하도록 확장한다.

## Scope
- `src/content/guidebook/en/**` — ko 콘텐츠를 en으로 번역한 markdown 파일 전체
- guidebook route(`src/app/[locale]/guidebook/`)와 sidebar가 `locale=en`일 때 `src/content/guidebook/en/`를 참조하도록 확장
...
## Key Files
- `src/content/guidebook/en/**`

## Notes
- **병렬 실행 가능**: 이 task는 `000006-010`(ja), `000006-030`(zh), `000006-040`(es)와 서로 disjoint한 디렉터리(`src/content/guidebook/en/`)만 소유하므로, 4개 locale task는 `000005-020` 완료 후 서로 병렬로 실행 가능하다.
...
### Task Verify
- `npm run build` — `/en/guidebook/*` 페이지가 정상 생성되는지 확인
- ko/en 파일 목록 1:1 대조

## Out of Scope
- guidebook route/sidebar의 구조적 변경 — `000004-020`이 이미 완료한 범위
- ja/zh/es 번역 — 각각 `000006-010`/`000006-030`/`000006-040`의 책임
