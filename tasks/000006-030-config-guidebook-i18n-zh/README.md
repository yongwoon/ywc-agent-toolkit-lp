# 000006-030-config-guidebook-i18n-zh

## Purpose
`src/content/guidebook/ko/`(이 task 실행 시점에 존재하는 모든 페이지)의 한국어 guidebook 콘텐츠를 중국어(zh)로 번역하고, `src/content/guidebook/zh/`에 배치하며, guidebook route/sidebar가 zh locale도 서빙하도록 확장한다.

## Scope
- `src/content/guidebook/zh/**` — ko 콘텐츠를 zh로 번역한 markdown 파일 전체
- guidebook route(`src/app/[locale]/guidebook/`)와 sidebar가 `locale=zh`일 때 `src/content/guidebook/zh/`를 참조하도록 확장
...
## Key Files
- `src/content/guidebook/zh/**`

## Notes
- **병렬 실행 가능**: 이 task는 `000006-010`(ja), `000006-020`(en), `000006-040`(es)와 서로 disjoint한 디렉터리(`src/content/guidebook/zh/`)만 소유하므로, 4개 locale task는 `000005-020` 완료 후 서로 병렬로 실행 가능하다.
- 번역 대상 페이지 수는 이 task 착수 시점의 `src/content/guidebook/ko/` 실제 파일 목록을 기준으로 한다.
- 기술 용어(Skill명, Agent명, 커맨드 예시 등)는 원문 그대로 유지하고 설명 문구만 번역한다. 간체/번체 여부는 `src/messages/zh.json`(Phase 000002에서 이미 확정된 LP 번역 카탈로그)의 표기 방식과 일관되게 맞춘다.
...
### Task Verify
- `npm run build` — `/zh/guidebook/*` 페이지가 정상 생성되는지 확인
- ko/zh 파일 목록 1:1 대조

## Out of Scope
- guidebook route/sidebar의 구조적 변경 — `000004-020`이 이미 완료한 범위
- ja/en/es 번역 — 각각 `000006-010`/`000006-020`/`000006-040`의 책임
