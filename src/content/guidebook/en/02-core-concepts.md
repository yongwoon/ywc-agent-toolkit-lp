...
`ywc-finish-branch`, `ywc-sequential-executor`, and `ywc-parallel-executor` share the mode concepts below for task-based delivery. The Small flow based on `plan.md` has no task artifact, so it opens a PR with `ywc-create-pr` and handles review/merge separately. Exact flag combinations are covered in [12. Executor / Code-gen Prompt patterns](./12-executor-and-codegen-patterns.md).
