...
다만 Skill 을 실제로 실행할 때는 `git`, `gh`, `python3` 등 몇 가지 Tool 이 System 에 준비되어 있어야 하고, 디자인 관련 Skill 을 쓰려면 별도 설정이 도움이 됩니다. 필수 Tool 과 선택 Tool 목록, 설치 방법은 [15. 사전 준비와 설치](./15-prerequisites-installation.md)에 정리했습니다 — 시작하기 전에 한 번 훑어보는 것을 권장합니다.
...
`/` 를 앞에 붙인 `/ywc-plan` 형태도 동일하게 동작합니다 — 이 문서에서는 가독성을 위해 `/` 를 생략합니다. `--flag value` 형태의 옵션은 Skill 마다 다르며, 각 Skill 페이지 또는 [13. Executor / Code-gen Prompt 패턴](./13-executor-and-codegen-patterns.md)에 실제 예시로 정리되어 있습니다.
