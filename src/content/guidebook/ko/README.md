...
| 이미 분해된 Task 들을 실행하고 싶다 (순차/병렬) | `ywc-sequential-executor` / `ywc-parallel-executor` | [12](./12-executor-and-codegen-patterns.md) |
| 아이디어가 아직 구체적이지 않아 정리부터 하고 싶다 | `ywc-brainstorm` | [13](./13-skill-reference.md) |
...
| 코드에 보안 취약점이 없는지 점검하고 싶다 | `ywc-security-audit` | [13](./13-skill-reference.md) |
| 버그의 근본 원인을 못 찾아 답답하다 | `ywc-debug-rootcause` | [13](./13-skill-reference.md) |
| Production 장애가 나서 회고를 써야 한다 | `ywc-incident-postmortem` | [13](./13-skill-reference.md) |
| 오래된 dead code 를 정리하고 싶다 | `ywc-refactor-clean` | [13](./13-skill-reference.md) |
| 열려있는 PR 의 review comment 에 대응해야 한다 | `ywc-handle-pr-reviews` | [13](./13-skill-reference.md) |
| Dependabot PR 이 쌓여서 한번에 정리하고 싶다 | `ywc-merge-dependabot` | [13](./13-skill-reference.md) |
| CHANGELOG 나 release note 를 작성하고 싶다 | `ywc-changelog-release-notes` | [13](./13-skill-reference.md) |
| 지금까지 한 작업을 그냥 commit 하고 싶다 | `ywc-commit` | [13](./13-skill-reference.md) |

이 표에 없는 상황은 [13. 전체 Skill 레퍼런스](./13-skill-reference.md)에서 전체 목록을 확인하세요.
...
| 페이지 | 설명 |
|---|---|
| [12. Executor / Code-gen Prompt 패턴](./12-executor-and-codegen-patterns.md) | 옵션이 많은 `ywc-sequential-executor` / `ywc-parallel-executor` / `ywc-code-gen` 실전 명령어 모음 |
| [13. 전체 Skill 레퍼런스](./13-skill-reference.md) | 위 가이드에서 다루지 않은 나머지 Skill 을 상황별로 정리한 색인 |

## 원본 자료

이 Guidebook 의 모든 command syntax 와 옵션은 각 Skill 의 `SKILL.md` (`tools/claude-code/skills/<skill-name>/SKILL.md`)를 근거로 검증되었습니다. 옵션이 실제 동작과 다르게 느껴진다면 해당 Skill 의 `SKILL.md` 가 최신 기준입니다 — 이 문서는 그 내용을 사용자 친화적으로 재구성한 2차 자료입니다.
