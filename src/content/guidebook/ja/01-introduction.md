...
ただし Skill を実際に実行するときは `git`、`gh`、`python3` などいくつかの Tool が System に必要で、デザイン関連 Skill を使うなら別途セットアップしておくと安定します。必須 Tool と選択 Tool の一覧、インストール方法は [17. 事前準備とインストール](./15-prerequisites-installation.md) にまとめました — 始める前に一度目を通しておくことをおすすめします。
...
先頭に `/` を付けた `/ywc-plan` 形式でも同じように動作します。この文書では可読性のため `/` を省略します。`--flag value` 形式のオプションは Skill ごとに異なり、各 Skill ページまたは [15. Executor / Code-gen Prompt パターン](./13-executor-and-codegen-patterns.md)に実例として整理しています。
