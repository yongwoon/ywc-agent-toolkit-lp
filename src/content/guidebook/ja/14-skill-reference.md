...
まず `--dry-run` で Opportunity Backlog だけを確認し、問題なければ flag を外して実際の consolidation を実行します。codebase 全体を一度に対象にすることはできません(Scope Gate) — 必ず module/directory 単位で範囲を絞る必要があります。コードを変更せず legibility の問題だけを洗い出したい場合は `ywc-agent-legibility-audit` を、dead code の削除だけが必要な場合は `ywc-refactor-clean` を代わりに使います。
...
[04](./04-general-cycle-small.md)、[05](./05-general-cycle-medium-large.md) の PR 前検証ステップとしてすでに組み込まれていますが、この流れの外で、既存のコードを対象に architecture/design/devex/security/QA の 5 軸レビューだけを単独で受けたいときもそのまま使えます。コードを変更しない読み取り専用の分析です — 見つかった項目の実際の修正は Backend/Frontend 担当 agent に別途 dispatch します。
...
Correctness やセキュリティではなく、「この code を agent が安全に修正するのにどれだけ token コストがかかるか」を deep/shallow module 比率と change-point の明示性を基準に測定する読み取り専用のレポートです。構造そのものは実際には修正しません。見つかった項目は `ywc-improve-architecture`(shallow→deep 再構造化) または `ywc-refactor-clean`(dead code 削除) に routing されます。
...
| Skill | 説明 | 場所 |
|---|---|---|
| `ywc-agent-legibility-audit` | Agent がコードを修正する際にかかる token コストと可読性を測定したい | [こちら](#agent-がコードを修正する際にかかる-token-コストと可読性を測定したい) |
...
| `ywc-code-gen` | Backend/Frontend/QA の 3 layer を並列に同時生成する多層 code 生成 skill | [13](./13-executor-and-codegen-patterns.md) |
...
| `ywc-impl-review` | 一般 cycle の外で、独立して実装品質と maintenance 観点を点検したい | [こちら](#一般-cycle-の外で独立して実装品質と-maintenance-観点を点検したい) |
| `ywc-improve-architecture` | 絡み合った shallow module 構造を deep module に再構造化したい | [こちら](#絡み合った-shallow-module-構造を-deep-module-に再構造化したい) |
...
| `ywc-parallel-executor` | 複数の Task を隔離された worktree で並列実行する executor | [13](./13-executor-and-codegen-patterns.md) |
...
| `ywc-refactor-clean` | 古い dead code (未使用関数/export/依存関係)を整理したい | [こちら](#古い-dead-code-未使用関数export依存関係を整理したい) |
...
| `ywc-sequential-executor` | 複数の Task を順番に 1 つずつ実行する executor | [13](./13-executor-and-codegen-patterns.md) |
...
[← 前: 13. Executor / Code-gen Prompt パターン](./13-executor-and-codegen-patterns.md) · [次: 15. 事前準備とインストール →](./15-prerequisites-installation.md)
