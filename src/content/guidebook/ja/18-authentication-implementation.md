...
| 監査結果 | 次に起きること |
|---|---|
| Critical/High が 0 件 | 承認済みのフロー(メール/パスワードを選んだ場合のみサインアップ/サインイン/リセット、有効な場合のみアカウント削除、設定した OAuth provider ごとに 1 フロー、承認されていれば MFA)だけを対象にした `ywc-e2e-test-strategy` によるポリシー条件付きの E2E に進む |
| Critical/High が 1 件でもある | `DONE_WITH_CONCERNS` を返し、是正されるまで E2E と PR 作成をスキップする |
