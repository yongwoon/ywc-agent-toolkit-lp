...
`ywc-finish-branch`, `ywc-sequential-executor`, `ywc-parallel-executor` は、task ベースの delivery で以下のモード概念を共有します。`plan.md` ベースの Small フローには task artifact がないため、`ywc-create-pr` で PR を開き、その後の review/merge は別途処理します。正確な flag の組み合わせは [13. Executor / Code-gen Prompt パターン](./13-executor-and-codegen-patterns.md)で扱います。
