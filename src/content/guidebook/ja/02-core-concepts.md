...
| 用語 | 意味 |
|---|---|
| **Skill** | `/ywc-plan` のようにユーザーが直接呼び出す手順。`tools/claude-code/skills/<name>/SKILL.md` に定義されており、この Guidebook が扱う対象の大半です。 |
...
`ywc-finish-branch`, `ywc-sequential-executor`, `ywc-parallel-executor` は、task ベースの delivery で以下のモード概念を共有します。`plan.md` ベースの Small フローには task artifact がないため、`ywc-create-pr` で PR を開き、その後の review/merge は別途処理します。正確な flag の組み合わせは [12. Executor / Code-gen Prompt パターン](./12-executor-and-codegen-patterns.md)で扱います。
